class Set:
  def __init__(self, capacity):
    # allow enough slots for the specified capacity, but track the actual number of elements separately.
  ...
  def __iter__(self):
    # Somebody is about to iterate through the set, initialize an index to begin.
    print("iter called. initializing iter_index to 0")
    self.__iter_index = 0
    return self

  def __next__(self):
    # If there are no more values left, terminate the iteration. 
    # Python does this automatically when it receives the exception generated here.
    print("next called while iter_index is", self.__iter_index)
    if self.__iter_index == self.__size:
      raise StopIteration
    # Each time a new value is requested (such as a cycle in a for loop),
    # grab the value, up the index to prepare for the next call, and return the value.
