...
Data new_Data(const char *fname) {
    ...
    data->dimensionality = malloc(sizeof(struct shape));
    ...
    data->targets = (int*) malloc(example_count * sizeof(int));
    ...
    while (misclassified) {
        ...
        for (int row=0; row<example_count; row++) {
            ...
            if ((hypothesis > 0 && target > 0) || (hypothesis < 0 && target < 0) || target==0) {
                ...
                // Save misclassified point to temporary data on the stack
                Data misclassified_data;
                struct data temp_data;
                misclassified_data = &temp_data;
                misclassified_data->inputs = &data->inputs[row];
                misclassified_data->targets = &data->targets[row];
...
void evaluate_model(Model model, Data data) {
    ...
    for (int row=0; row < data->dimensionality->example_count; row++) {
        hypothesis = predict(model, data->inputs[row]);
        target = data->targets[row];
        if ((hypothesis > 0 && target > 0) || (hypothesis < 0 && target < 0) || target==0) {
            correct_count++;
    ...
    double percent_correct = correct_count / data->dimensionality->example_count;
    printf("Testing Results: model is %f%% correct \n", 100*percent_correct); 
}
...
void delete_Data(Data data) {
    free(data->targets);
    for (int row=0; row < data->dimensionality->example_count; row++) {
        free(data->inputs[row]);
    }
    free(data->inputs);
    free(data->dimensionality);
    free(data);
}

void delete_Model(Model model) {
    free(model->dimensionality);
    free(model->weights);
    free(model);
}
